...
extern "C" void initNativeFileSystemMonitor()
{
    ...
    if (sm == NULL) {
        sm = new MidpFileSystemMonitor(mr);
    }
}
...
void MidpMountedRoots::update(bool notify)
{
    ...
    int   indexes[mMaxRootsSize];
    char* paths[mMaxRootsSize];
    ...
    if (mntfp) {
        while ((me = getmntent(mntfp)) != 0) {
            const int idx = getDiskIndex(me->mnt_fsname);
            if (idx >= 0) {
                count++;

                indexes[count - 1] = idx;

                char* path = (char*)midpMalloc(strlen(me->mnt_dir) + 2);
                strcpy(path, me->mnt_dir);
                strcat(path, "/");
                paths[count - 1] = path;
