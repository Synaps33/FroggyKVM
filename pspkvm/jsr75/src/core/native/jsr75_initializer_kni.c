...
SUITESORE_DECLARE_LISTENER(jsr75_remove_listener);
...
Java_com_sun_midp_jsr075_Initializer_init() {
    (void) midp_suite_add_listener(jsr75_remove_listener,
 ...
 */
KNIEXPORT KNI_RETURNTYPE_VOID
Java_com_sun_midp_jsr075_Initializer_cleanup() {
    (void) midp_suite_remove_listener(jsr75_remove_listener,
                                      SUITESTORE_LISTENER_TYPE_REMOVE);
    KNI_ReturnVoid();
}
 ...
 * is performed
 */
SUITESORE_DEFINE_LISTENER(jsr75_remove_listener) {
    (void) listenerType;
    (void) when;

    if (status == ALL_OK) {
        jsr75_suite_remove_cleanup(pSuiteData->suiteId);
    }
}
