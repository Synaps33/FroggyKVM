    ...
    do {
        ...
        if (level >= LIST_LEVEL && initLevel < LIST_LEVEL) {
            ...
            if (err == 0) {
                ...
                fprintf(stderr, "[GB300-DEBUG] storageInitialize failed! err=%d, midpConfig=%s, midpHome=%s\n", err, midpConfig ? midpConfig : "null", midpHome ? midpHome : "null");
                status = OUT_OF_MEMORY;
            }

            if (status != ALL_OK) {
                fprintf(stderr, "[GB300-DEBUG] midp_suite_storage_init failed or storageInitialize failed! status=%d\n", status);
                break;
            }

            if (initializeConfig() != 0) {
                fprintf(stderr, "[GB300-DEBUG] initializeConfig() failed!\n");
                break;
            }
