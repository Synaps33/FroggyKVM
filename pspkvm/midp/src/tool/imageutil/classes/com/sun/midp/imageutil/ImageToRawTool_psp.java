 ...
 * Main tool class
 */
public class ImageToRawTool_psp {
    /**
    ...
    public static void main(String[] args) {
        try {
            ...
            ImageToRawTool_psp tool = new ImageToRawTool_psp();
    ...
    private void convertImageToRaw(String sourceName, String destName)
        ...
        writer.write("0x"+Integer.toHexString(width).toUpperCase()+ ", ");
        writer.write("0x"+Integer.toHexString(height).toUpperCase()+ ", ");
        writer.write("\r\n");
        
        for (int i=16; i <rawData.length; ) {
            for (int j = 0; (j < 64) && (i < rawData.length); j++, i+=2) {
            	  int h = rawData[i+1];
            	  int l = rawData[i];
            	  int v = ((h<<8) & 0xff00) | (l & 0xff);
                writer.write("0x"+Integer.toHexString(v).toUpperCase()+ ", ");
            }
            writer.write("\r\n");
        }
        writer.close();
    ...
    private int readShort(byte[] data, int offset) {
    	int h, l;
    	if (endianFormat == ImageToRawConverter.INT_FORMAT_BIG_ENDIAN) {
    	  h = data[offset];
         l = data[offset+1];
    	} else {
    	  h = data[offset+1];
