...
#define DEBUGP2F(x, y) { \
    const char* pszTemp = pcsl_string_get_utf8_data(y); \
        if (pszTemp != NULL) { \
            REPORT_ERROR1(LC_CORE, x, pszTemp); \
...
storage_rename_file(char** ppszError, const pcsl_string* oldFilename_str,
    ...
    *ppszError = NULL;
    ...
    if ((status = pcsl_file_open(oldFilename_str, PCSL_FILE_O_RDONLY, &rd)) == 0) {
        if ((status =  pcsl_file_open(newFilename_str, PCSL_FILE_O_WRONLY | PCSL_FILE_O_CREAT, &wd)) == 0) {
            do {
                readbytes = pcsl_file_read(rd, buf, sizeof(buf));
                status = pcsl_file_write(wd, buf, readbytes);
            } while (readbytes > 0 && status >= 0);
            pcsl_file_close(rd);
            pcsl_file_close(wd);
        } else {
            pcsl_file_close(rd);
