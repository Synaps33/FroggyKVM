...
bool Universe::bootstrap_with_rom(const JvmPathChar* classpath) {
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling ROM::link_static");
  ...
  // NOTE: last <NUM_HANDLES_SKIP> handles not ROMIZED.
  {
    ...
    tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> ROM::link_static returned %d", (int)status);
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> setting up waiting thread");
  ...
  *(OopDesc**)scheduler_waiting() =
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling StackmapGenerator::initialize");
  StackmapGenerator::initialize(_gc_stackmap_size JVM_MUST_SUCCEED);
  
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling init_task_list");
  // Allocate task list and the system Task object
  init_task_list(JVM_SINGLE_ARG_MUST_SUCCEED);

  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling ObjectHeap::finish_system_allocation");
  // The rest stuff is allocated in the context of new task
  ObjectHeap::finish_system_allocation(Task::FIRST_TASK);
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling allocate_bootstrap_thread");
  allocate_bootstrap_thread();
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling create_first_task");
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling update_relative_pointers");
  update_relative_pointers();

  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling setup_thread_priority_list");
  setup_thread_priority_list(JVM_SINGLE_ARG_MUST_SUCCEED);
  setup_thread(Thread::current()); // Now set up Thread::current()->klass()

  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling create_main_thread_mirror");
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling Scheduler::start");
  Scheduler::start(Thread::current() JVM_MUST_SUCCEED);
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> calling initialize_main");
  Thread::current()->initialize_main(JVM_SINGLE_ARG_MUST_SUCCEED);
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_with_rom -> done");
...
bool Universe::bootstrap_without_rom(const JvmPathChar* classpath) {
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> calling allocate_bootstrap_thread");
  allocate_bootstrap_thread();
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> calling create_meta");
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> calling Thread::current()->initialize_main");
  ...
  *mp_stack_list() = Universe::new_obj_array(16 JVM_CHECK_0);
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> locking obj table");
  // lock table for interned Strings
  *lock_obj_table() = Universe::new_obj_array(4 JVM_CHECK_0);

  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> allocating class list");
  // Allocate tables used for class loading
  *class_list() = new_obj_array(200 JVM_CHECK_0);

  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> allocating global refs array");
  // Global reference support
  *global_refs_array() = Universe::new_int_array(5 JVM_CHECK_0);

#if ENABLE_ISOLATES
  *mirror_list() = setup_mirror_list(200 JVM_CHECK_0);
#endif
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> calling init_task_list");
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> registering object_array_class");
  ...
  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> calling Symbols::initialize");
  // Initialize global VM symbols (must be done after classes such as "[C"
  // have been registered.
  Symbols::initialize(JVM_SINGLE_ARG_CHECK_0);
  if (CURRENT_HAS_PENDING_EXCEPTION) {
      fprintf(stderr, "[GB300-DEBUG] Symbols::initialize returned with pending exception!\n");
      fflush(stderr);
  }


  tty->print_cr("[GB300-DEBUG] Universe::bootstrap_without_rom -> calling Bytecodes::verify");
  // Verify bytecodes.
  Bytecodes::verify();
...
ReturnOop Universe::new_task(int id JVM_TRAPS) {
  UsingFastOops fast_oops;
  tty->print_cr("[GB300-DEBUG] new_task -> entered id=%d", id);
  Task::Fast task = allocate_task(JVM_SINGLE_ARG_CHECK_0);
  tty->print_cr("[GB300-DEBUG] new_task -> allocate_task done");
  ...
  tty->print_cr("[GB300-DEBUG] new_task -> num_classes=%d", num);
  ...
  if (id == 0) {
    ...
    tty->print_cr("[GB300-DEBUG] new_task -> system task done");
    return task;
  }

  {
    tty->print_cr("[GB300-DEBUG] new_task -> creating class list");
    ...
    tty->print_cr("[GB300-DEBUG] new_task -> class list done");
  }
  { 
...
#endif
    {
      tty->print_cr("[GB300-DEBUG] new_task -> creating mirror list");
      ml = Universe::new_obj_array(num + pad JVM_CHECK_0);          
      const int len = ml().length();
      tty->print_cr("[GB300-DEBUG] new_task -> fetching task_class_init_marker");
      TaskMirror::Raw t = task_class_init_marker();
      tty->print_cr("[GB300-DEBUG] new_task -> filling mirror list len=%d");
      for (int i = 0; i < len; i++) {
        ml().obj_at_put(i, t);
      }
      tty->print_cr("[GB300-DEBUG] new_task -> mirror list done");
    }
    task().set_mirror_list(ml);
  } 
  {
    tty->print_cr("[GB300-DEBUG] new_task -> creating dictionary");
    ObjArray::Raw task_dictionary = Universe::new_obj_array(64 JVM_CHECK_0);
    task().set_dictionary(task_dictionary);
  }
  {
    tty->print_cr("[GB300-DEBUG] new_task -> creating string table");
    StringTable::Raw table = StringTable::initialize(64 JVM_CHECK_0);
    task().set_string_table(table);
  }
  {
    tty->print_cr("[GB300-DEBUG] new_task -> creating symbol table");
    SymbolTable::Raw table = SymbolTable::initialize(64 JVM_CHECK_0);
    task().set_symbol_table(table);
  }
  {
    tty->print_cr("[GB300-DEBUG] new_task -> creating global refs");
...
#endif // ENABLE_MULTIPLE_PROFILES_SUPPORT
  tty->print_cr("[GB300-DEBUG] new_task -> calling update_relative_pointers");
  update_relative_pointers();
  tty->print_cr("[GB300-DEBUG] new_task -> done");
  return task;
}
#endif
...
void Universe::update_relative_pointers() {
  ...
  // Use per-task mirror_list (properly filled with init markers) rather than
  // the global Universe::mirror_list() which is uninitialized.
  {
    Task::Raw current = Task::current();
    if (current.not_null()) {
      ObjArray::Raw ml = current().mirror_list();
      if (ml.not_null()) {
        _mirror_list_base = (address)ml.obj();
