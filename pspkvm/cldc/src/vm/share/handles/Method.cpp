...
void Method::check_bytecodes(JVM_SINGLE_ARG_TRAPS) {
...

  {
    ...
    for (bci = 0; bcptr < bcend;) {
      ...
      if (code < Bytecodes::_tableswitch) {
        ...
        if ((bytecode_length = Bytecodes::length_for(code)) < 0) {
          ...
          fprintf(stderr, "[GB300-DEBUG] iterate_bytecodes failed, reason 1. code=%d\n", code); fflush(stderr); goto error;
        }
      ...
      if (code >= Bytecodes::_breakpoint ||!Bytecodes::is_defined(code)) {
        ...
        fprintf(stderr, "[GB300-DEBUG] iterate_bytecodes failed, reason 2. code=%d\n", code); fflush(stderr); goto error;
      }
      else if (code == Bytecodes::_wide) {  // 0xc4
        code = bytecode_at(bci + 1);
        if (!Bytecodes::is_defined(code)) {
          fprintf(stderr, "[GB300-DEBUG] iterate_bytecodes failed, reason 3. code=%d\n", code); fflush(stderr); goto error;
        }
        ...
        case Bytecodes::_tableswitch:      // 0xaa
          {
            ...
            for (int offset = bci; ++offset < a_bci; ) {
              if ((jubyte)bytecode_at_raw(offset) != 0) {
                fprintf(stderr, "[GB300-DEBUG] iterate_bytecodes failed, reason 4. offset=%d, byte=%d\n", offset, (int)(jubyte)bytecode_at_raw(offset)); fflush(stderr); goto error;
              }
            }
            int fields;
            if (code == Bytecodes::_tableswitch) {
            ...
            if (end >= codesize || fields < 0 || fields > codesize) {
              fprintf(stderr, "[GB300-DEBUG] iterate_bytecodes failed, reason 5. end=%d, codesize=%d, fields=%d\n", end, codesize, fields); fflush(stderr); goto error;
            }
            if (Bytes::is_Java_byte_ordering_different()
        ...
        if ((bytecode_length = bytecode_length_for(bci)) == -1) {
          fprintf(stderr, "[GB300-DEBUG] iterate_bytecodes failed, reason 6. bci=%d\n", bci); fflush(stderr); goto error;
        }
      }
      bci += bytecode_length;
      bcptr += bytecode_length;
    ...
    if (bci > codesize) {
      fprintf(stderr, "[GB300-DEBUG] iterate_bytecodes failed, reason 7. bci=%d, codesize=%d\n", bci, codesize); fflush(stderr); goto error;
    }
...
error:
  fprintf(stderr, "[GB300-DEBUG] Method iterate_bytecodes failed!\n");
  fflush(stderr);
