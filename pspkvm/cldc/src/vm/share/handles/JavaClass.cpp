...
void JavaClass::set_array_class(ObjArrayClass* value JVM_TRAPS) {
  ...
  if ((GenerateROMImage && !ENABLE_MONET)
    ...
    // During early bootstrap, java_lang_Class_class is null so no TaskMirrors
    // exist yet. Fall back to storing directly in the field like the non-ROM path.
    if (Universe::java_lang_Class_class()->is_null() || is_fake_class()) {
      obj_field_put(array_class_offset(), value);
      JVM_IGNORE_TRAPS;
      return;
    }
    ...
    TaskMirror::Raw tm = task_mirror_no_check();
    if (tm.is_null() && !is_instance_class()) {
    ...
    if (!(GenerateROMImage && !ENABLE_MONET) &&
      ...
      if (self().is_being_initialized_mirror(&tm)) {
        ...
        if (self().is_instance_class()) {
          ...
          // static fields with initializers anyway.
          if (tm.not_null() && !self().is_fake_class()) {
            InstanceClass::Raw ic = self().obj();
