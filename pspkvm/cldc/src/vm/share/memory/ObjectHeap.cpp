...
bool ObjectHeap::adjust_heap_size(size_t target_heap_size) {
  ...
  const size_t extra_size = align_size_up(_preallocated_space_size + _glue_code_size, 128);
  ...
  tty->print_cr("[GB300-DEBUG] ObjectHeap init: heap_chunk=0x%x extra=0x%x heap_start=0x%x bitv_start=0x%x bitv_base=0x%x",
          (unsigned int)_heap_chunk, (unsigned int)extra_size, (unsigned int)_heap_start, (unsigned int)_bitvector_start, (unsigned int)_bitvector_base);
...
void ObjectHeap::find(OopDesc *target, bool verbose_owner) {
  for (OopDesc** ptr = _heap_start; ptr < _inline_allocation_top; ptr++) {
    if (*ptr == target) {
      ...
      if (verbose_owner) {
        VerbosePointers = false;
        Oop::Raw oop = owner;
        tty->print(" ");
        oop().print_value_on(tty);
        VerbosePointers = true;
      }
      tty->cr();
    }
  }
