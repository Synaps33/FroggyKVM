...
int JVM::start() {
  ...
  tty->print_cr("[GB300-DEBUG] JVM::start -> calling initialize");
  if ((ok = initialize()) == false) {
    tty->print_cr("[GB300-DEBUG] JVM::start -> initialize failed");
    goto done;
  }
  
  tty->print_cr("[GB300-DEBUG] JVM::start -> calling check_misc_options");
  ok = check_misc_options(JVM_SINGLE_ARG_NO_CHECK);
  if (!ok) {
    tty->print_cr("[GB300-DEBUG] JVM::start -> check_misc_options failed");
    goto done;
  }
