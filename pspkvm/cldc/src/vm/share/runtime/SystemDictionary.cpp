...
ReturnOop SystemDictionary::fetch_buffer(LoaderContext *loader_ctx JVM_TRAPS) {
  ...
  if (fd.not_null()) {
    if (UseROM && 
      ...
      tty->print_cr("[GB300-DEBUG] SystemDictionary::fetch_buffer throwing class_not_found due to restricted package");
                                         ...
                                         InstanceClass *fake_cls JVM_TRAPS)
{
  ...
  if (ac.not_null()) {
    ...
    // a null TaskMirror. We must also enter this block when rom_num==0.
