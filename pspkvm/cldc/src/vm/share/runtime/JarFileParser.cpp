...
bool JarFileParser::find_entry(const char *match_name JVM_TRAPS)
{
  ...
  while (true) {
    ...
    if (jar_buffer().seek(offset, SEEK_SET) < 0) {
      tty->print_cr("[GB300-DEBUG] find_entry: seek(%d) failed -> return false", offset);
      return false;
    }
    /* Read the bytes */
    if (jar_buffer().get_bytes(cenp, CENHDRSIZ) != CENHDRSIZ) {
      tty->print_cr("[GB300-DEBUG] find_entry: get_bytes CENHDRSIZ failed at offset=%d -> return false", offset);
      return false;
    }
    /* p contains the current central header */
    if (GETSIG(cenp) != CENSIG) {
      /* We've reached the end of the headers */
      tty->print_cr("[GB300-DEBUG] find_entry: GETSIG mismatch at offset=%d, got 0x%x, expected 0x%x -> return false",
          offset, (unsigned int)GETSIG(cenp), (unsigned int)CENSIG);
      return false;
    }
