...
juint Inflater::crc32(const unsigned char *data, const juint length) {
  ...
  tty->print_cr("[GB300-DEBUG] Inflater::crc32 called with data=0x%08x, length=%d", (unsigned int)data, length);

#if ENABLE_FAST_CRC32
  tty->print_cr("[GB300-DEBUG] Inflater::crc32 using FAST_CRC32");
...
#else
  tty->print_cr("[GB300-DEBUG] Inflater::crc32 using SLOW_CRC32");
  unsigned int j;
  ...
  tty->print_cr("[GB300-DEBUG] Inflater::crc32 returning 0x%08x", ~crc);
  return ~crc;
}
#endif
