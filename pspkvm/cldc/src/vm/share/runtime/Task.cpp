...
ReturnOop Task::allocate_task(int id JVM_TRAPS) {
  ...
  tty->print_cr("[GB300-DEBUG] allocate_task: task_list()=0x%x, id=%d, task.obj=0x%x, obj_at_id=0x%x",
     (unsigned int)Universe::task_list()->obj(), id, (unsigned int)task.obj(),
     (unsigned int)Universe::task_list()->obj_at(id));
  
  // GC may have run during new_task() and moved the task list array,
  // making _raw_task_list stale. Re-sync it from the GC-updated handle.
