...
jushort ClassFileParser::get_u2(JVM_SINGLE_ARG_TRAPS) {
  jushort val = Bytes::get_Java_u2((address)_bufptr);
  _bufptr += 2;
  if (_bufptr > _bufend) {
    truncated_class_file_error(JVM_SINGLE_ARG_NO_CHECK_AT_BOTTOM);
    return 0;
      ...
      while (tableptr < tableend) {
        ...
        *tableptr++ = exception_index;
        if (exception_index != 0 && !cp->check_klass_at(exception_index)) {
           fprintf(stderr, "[GB300-DEBUG] exception table catch block tag check FAILED! index=%d, tag=%d\n", exception_index, (int)cp->tag_value_at(exception_index));
           fflush(stderr);
           goto invalid;
        }
      }
  ...
  while (method_attributes_count--) {
    ...
    if (method_attribute_name.equals(Symbols::tag_code())) {
      ...
      for (int i = 0; i < checked_exceptions_length; i++) {
        ...
        if (!cp->check_klass_at(exception_index)) {
           fprintf(stderr, "[GB300-DEBUG] exception tag check FAILED! index=%d, tag=%d\n", exception_index, (int)cp->tag_value_at(exception_index));
           fflush(stderr);
        }
...
error:
  fprintf(stderr, "[GB300-DEBUG] check_for_duplicate_methods failed!\n");
  fflush(stderr);
  classfile_parse_error(duplicate_field JVM_NO_CHECK_AT_BOTTOM);
}
...
inline bool ClassFileParser::is_circular(InstanceClass* this_class) {
  for( InstanceClass::Raw c = this_class->obj();; ) {
    c = c().super();
    if( c().is_null() ) {
      return false;
    }
    if (c.equals(this_class->obj())) {
      return true;
    }
  }
}
...
ClassFileParser::check_local_interfaces(InstanceClass* this_class JVM_TRAPS) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    cpf_check(!interf.equals(this_class->obj()) &&
...
ReturnOop ClassFileParser::parse_class_internal(ClassParserState *stack JVM_TRAPS) {
  ...
  this_class().set_super(super_class.obj());
