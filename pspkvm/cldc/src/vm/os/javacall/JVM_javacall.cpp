...
extern "C" int JVM_Start(const JvmPathChar *classpath, char *main_class, int argc,
                         char **argv) {
  fprintf(stderr, "[GB300-DEBUG] JVM_Start entered. calling set_arguments...\n");
  JVM::set_arguments(classpath, main_class, argc, argv);

  int result = 0;
  /* Try print some log info. and start VM */

  fprintf(stderr, "[GB300-DEBUG] JVM_Start calling JVM::start()...\n");
  result = JVM::start();
  fprintf(stderr, "[GB300-DEBUG] JVM_Start returned from JVM::start() with result=%d\n", result);
