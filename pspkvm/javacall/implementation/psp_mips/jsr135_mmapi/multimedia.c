...
/** Declaration of MP3 functions **/
static mmplayer_handle* create_mp3(javacall_int64 playerId, const javacall_utf16* uri, long uriLength, long contentLength);
static void close_mp3(mp3_player_handle* mp);
static long buffer_mp3(mp3_player_handle* mp, const void* buffer, long length, long offset);
...
static void channelDone(int channel)
{
    ...
    javacall_printf("channel %d finished playing.\n", channel);
    player = wav_players;
    while (player != NULL) {
...
javacall_handle javacall_media_create(javacall_int64 playerId, 
                                      ...
                                      long contentLength)
{
    javacall_printf("javacall_media_create, contentLength=%d\n", contentLength);
    ...
    if (!strcmp(pszMime, JAVACALL_AUDIO_MIDI_MIME) ||
         ...
         !strcmp(pszMime, JAVACALL_AUDIO_WAV_MIME_2)) {
...
javacall_result javacall_media_close(javacall_handle handle) {
    ...
    if (mp) {
    	 ...
    	 } else if (mp->type == MEDIA_TYPE_MP3) {
    	     close_mp3((mp3_player_handle*)mp);
    	 }
    }
...
javacall_result javacall_media_protocol_handled_by_device(javacall_handle handle) {
    ...
    if (mp->type == MEDIA_TYPE_MIDI) {
        ...
        needBuffer = ((mp3_player_handle*)mp)->needBuffer;
    }
...
javacall_result javacall_media_start(javacall_handle handle) {
    ...
    if (mp->type == MEDIA_TYPE_MIDI) {
        ...
        return start_mp3((mp3_player_handle*)mp);
    }
    return JAVACALL_FAIL;
}
...
javacall_result javacall_media_pause(javacall_handle handle) {
    ...
    if (mp->type == MEDIA_TYPE_MIDI) {
        ...
        return pause_mp3((mp3_player_handle*)mp);
    }
    return JAVACALL_FAIL;
}
...
javacall_result javacall_media_resume(javacall_handle handle) {
    ...
    if (((mp3_player_handle*)handle)->type == MEDIA_TYPE_MP3 && 
    	  !((mp3_player_handle*)handle)->paused) {
    	  return JAVACALL_OK;
    }
...
javacall_result javacall_media_to_foreground(javacall_handle handle,
    ...
    if (option != JAVACALL_MEDIA_PLAYABLE_FROM_BACKGROUND) {
    	javacall_printf("javacall_media_to_foreground\n");
...
static void parse_file_url(const char* pszUri, /*OUTPUT*/char* filename) {
        static javacall_utf16 javaroot[32];
        static javacall_utf16 nativeroot[JAVACALL_MAX_FILE_NAME_LENGTH];
        int i, j;

        javacall_printf("parse_file_url: from file url %s\n", pszUri);

	filename[0] = '\0';

	if (pszUri[7] == '/') {
	    //It must be initial by "file:///"
	    pszUri+=8;
           for (i = 0; i< sizeof(javaroot)/sizeof(javacall_utf16); i++) {
           	javaroot[i] = (javacall_utf16)*pszUri;
           	pszUri++;
           	if (javaroot[i] == 0 || javaroot[i] == '/') break;           	
           }
                   
           if (JAVACALL_OK == javacall_fileconnection_get_path_for_root(javaroot, i+1, nativeroot, 
   	    	                                        sizeof(nativeroot)/sizeof(javacall_utf16) - 1)) {
   	      	for (i = 0; i < JAVACALL_MAX_FILE_NAME_LENGTH; i++) {
   	     		filename[i] = (char)nativeroot[i];
   	     		if (nativeroot[i] == 0) break;
   	      	}
   	      	
   	      	for (j = i; j < JAVACALL_MAX_FILE_NAME_LENGTH; j++) {
   	      		filename[j] = (javacall_utf16)*pszUri;
   	      		if (*pszUri == 0) break;
   	      		else pszUri++;
   	      	}
           }        

           javacall_printf("parse_file_url: file name:%s\n", filename);
	}
}
/*****************
 *  MIDI functions
...
static mmplayer_handle* create_midi(javacall_int64 playerId, const javacall_utf16* uri, long uriLength, long contentLength) {
    ...
    if (uri == NULL || uriLength <= 0) {
       ...
       if (!strncmp(pszUri, "http://", 7) && strlen(pszUri) < 256) {
           ...
           javacall_printf("Not supported URI:%s\n", pszUri);
           return 0;
       } else {
           needBuffer = 0;
       }
    ...
    if (pszUri == NULL) {
        handle->filename[0] = '\0';
    } else if (!strncmp(pszUri, "file://", 7)) {
        parse_file_url(pszUri, &handle->filename[0]);
    }  else {
...
static long buffer_midi(midi_player_handle* mp, const void* buffer, long length, long offset) {
    ...
    if (mp->fp) {
        ...
        javacall_printf("Buffer midi %d bytes from %d\n", length, offset);
    ...
    javacall_printf("%d bytes buffered\n", ret);
    return ret;
}
...
static long buffer_wav(wav_player_handle* mp, const void* buffer, long length, long offset) {
    ...
    if (mp->fp) {
        ...
        javacall_printf("Buffer wav %d bytes from %d\n", length, offset);
        memcpy(mp->buffer+offset, buffer, length);
        mp->buffered_len += length;
        ret = length;
    } else {
...
static javacall_result start_wav(wav_player_handle* mp) {
    Mix_Chunk * chunk;
    javacall_printf("start_wav\n");
    if (!mp->chunk && !mp->needBuffer) {
    	javacall_printf("Mix_LoadWAV...\n");
        chunk = Mix_LoadWAV(mp->filename);
    } else { 
        chunk = mp->chunk;
    }
    ...
    if (chunk) {
    	...
    	if ((mp->channel = Mix_PlayChannel(mp->channel, chunk, 0)) == -1) {
    	    javacall_printf("Mix_PlayChannel: %s\n",Mix_GetError());
    	    return JAVACALL_FAIL;
    	} else {
    	    return JAVACALL_OK;
    	}
...
static mmplayer_handle* create_mp3(javacall_int64 playerId, const javacall_utf16* uri, long uriLength, long contentLength) {
    int needBuffer;
    mp3_player_handle* handle;
    char *pszUri = NULL;

    javacall_printf("create_mp3>>\n");
    
    if (!mp3codec_enabled) {
    	javacall_printf("mp3 modules are not loaded!\n");
    	return NULL;
    }

    if (uri == NULL || uriLength <= 0) {
    	//Read from stream, need buffer
    	javacall_printf("create_mp3: read from stream\n");
    	needBuffer = 1;
    } else {
       pszUri = javacall_UNICODEsToUtf8(uri, uriLength);
       if (!strncmp(pszUri, "http://", 7) && strlen(pszUri) < 256) {
           javacall_printf("create_mp3: from url %s\n", pszUri);
           needBuffer = 1;
       } else if (strlen(pszUri) >= 256 || strncmp(pszUri, "file://", 7)) {
           javacall_printf("Not supported URI:%s\n", pszUri);
           return 0;
    ...
    handle = malloc(sizeof(mp3_player_handle));
    memset(handle, 0, sizeof(mp3_player_handle));
